export default function BreakdownBars({ label, data }: { label: string; data: Record<string, number> }) {
  const entries = Object.entries(data);
  const max = Math.max(1, ...entries.map(([, v]) => v));
  return (
    <div className="border border-gray-200 rounded-md p-4 bg-white">
      <div className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">{label}</div>
      {entries.length === 0 && <div className="text-sm text-gray-400">No data</div>}
        ...
        {entries.map(([key, value]) => (
          <div key={key} className="flex items-center gap-2">
            ...
            <div className="flex-1 bg-gray-100 rounded h-3">
              <div className="bg-blue-500 h-3 rounded" style={{ width: `${(value / max) * 100}%` }} />
            </div>
