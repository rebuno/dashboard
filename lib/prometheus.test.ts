...
describe("parsePrometheusText", () => {
  it("parses metrics without labels", () => {
    ...
    expect(samples).toEqual([
      { name: "rebuno_executions_created_total", labels: {}, value: 42 },
    ]);
  });
  ...
  it("parses metrics with labels", () => {
    ...
    expect(samples).toEqual([
      {
        name: "rebuno_executions_completed_total",
  ...
  it("skips comment lines and blank lines", () => {
    ...
    expect(samples).toEqual([
      { name: "rebuno_queue_depth", labels: {}, value: 3 },
    ]);
  });
});
...
describe("groupByLabel", () => {
  it("sums values grouped by a label", () => {
    ...
    expect(
      groupByLabel(samples, "rebuno_steps_submitted_total", "kind"),
    ).toEqual({
      tool_call: 5,
      llm_call: 3,
    });
...
describe("histogramQuantile", () => {
  it("computes p50 from cumulative buckets via linear interpolation", () => {
    ...
    const p50 = histogramQuantile(
      samples,
      "rebuno_dispatch_latency_seconds",
      0.5,
    );
  ...
  it("returns null when there are no buckets", () => {
    expect(
      histogramQuantile([], "rebuno_dispatch_latency_seconds", 0.5),
    ).toBeNull();
  });
});
